...
if (-not $wslInstalled) {
    ...
    if ($choice -match '^[sSyY]$') {
        ...
        if (-not $isAdmin) {
            ...
            $ScriptPath = Join-Path $ScriptRoot "Iniciar.bat"
            try {
                Start-Process cmd.exe -ArgumentList "/c `"$ScriptPath`"" -Verb RunAs -Wait
            } catch {
                Show-Error "Acceso Denegado" "La solicitud de elevacion fue cancelada o rechazada." "Debe ejecutar el lanzador como Administrador para poder instalar WSL."
            }
