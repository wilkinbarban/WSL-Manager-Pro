...
**Subsistema do Windows para Linux (WSL)**: listar distribuições, instalar
pelo catálogo online ou a partir de um rootfs baixado, importar/exportar,
provisionamento pós-instalação (conta de usuário, pacotes, `wsl.conf`),
limites de recursos por meio de `.wslconfig` e utilitários de manutenção.
...
- [Compilação e Distribuição](#compilação-e-distribuição)
...
| Compressão | **zstandard** (bootstrap `.tar.zst` do Arch Linux — evita depender de `tar --zstd` do sistema no Windows) |
| Formato de dados | **JSON** (`distros.json`, `config.json`, catálogos i18n) |
| Integração com o sistema | **subprocess** (`wsl.exe`, PowerShell, winget); **ctypes** (elevação UAC); **zipfile** / **tarfile** (manipulação de arquivos) |
| Estilo | Tema escuro via QSS (`resources/styles/dark.qss`) |
...
> 3. **Verifica o ambiente WSL** — Detecta se o WSL está instalado no sistema. Se estiver faltando, verifica a compatibilidade (OS Build >= 19041 e virtualização de hardware habilitada na BIOS) e solicita a instalação automática (UAC elevação, ativação de recursos do hipervisor e gerenciamento da reinicialização do sistema).
> 4. **Verifica o ambiente Python** — Detecta uma instalação compatível do Python (>=3.14). Caso falte, instala automaticamente o Python 3.14 usando o `winget` e atualiza o PATH do sistema.
> 5. **Prepara o ambiente virtual** — Cria ou recria automaticamente um ambiente virtual isolado `.venv` utilizando o Python local.
> 6. **Instala as dependências do projeto** — Atualiza o `pip` e instala silenciosamente todos os pacotes requeridos definidos no `requirements.txt` com um indicador de progresso interativo.
> 7. **Inicia o aplicativo** — Assim que todas as dependências forem verificadas, o script inicia o WSL Manager Pro automaticamente.
>
...
├── pyproject.toml              # Metadados, dependências, configuração do ruff e pytest
├── requirements.txt            # Dependências de execução
├── distros.json                # Catálogo estático de distros (URLs, gerenciadores de pacotes, pós-instalação)
├── ROADMAP.md                  # Plano de desenvolvimento multifase (150+ tarefas)
├── build.ps1                   # Disparador de build do PyInstaller (EXE único)
├── install.ps1                 # Instalador de ambiente com um clique
├── wsl_manager_pro.spec        # Arquivo spec do PyInstaller
├── wsl_manager_pro.rc          # Arquivo de recursos do Windows (incorporação de ícone)
│
├── core/                       # Lógica de negócio
│   ├── __init__.py
│   ├── wsl_engine.py           # Fachada sobre wsl.exe, PowerShell, pós-instalação, .wslconfig
│   ├── wsl_list_parser.py      # Parsers puros para saída do wsl --list (testáveis sem WSL)
│   ├── downloader.py           # Downloads HTTP com retomada + verificação de checksum
│   ├── catalog_loader.py       # Validação, carga e fusão remota de catálogos
│   └── constants.py            # Constantes de timeout, retentativas, chunks e limites de UI
│
├── utils/                      # Serviços transversais
│   ├── __init__.py
│   ├── config_manager.py       # Configuração JSON persistente
│   ├── app_logging.py          # Logger com arquivo rotativo
...
│   ├── __init__.py
│   ├── main_window.py          # QMainWindow: abas, toolbar, workers, logging
...
│   ├── theme.py                # Constantes de cor centralizadas para a UI
│   └── tabs/
│       ├── __init__.py
...
│   ├── i18n/
│   │   ├── en.json             # Traduções para inglês (300+ chaves)
│   │   ├── es.json             # Traduções para espanhol
│   │   └── pt.json             # Traduções para português (brasileiro)
│   └── styles/
│       └── dark.qss            # Folha de estilo escura do Qt (~250 linhas)
│
├── assets/                     # Ícones do aplicativo
│   ├── icon.ico
│   └── icon.png
│
├── tests/                      # Testes unitários (42 testes, não requerem WSL)
│   ├── __init__.py
│   ├── test_app_logging.py
...
1. **Configuração de path** — `main.py` insere a raiz do projeto em
   `sys.path` para que imports absolutos (`core`, `utils`, `ui`) funcionem
   independentemente do diretório de trabalho.
2. **Fallback para venv** — Se o PySide6 não estiver disponível no interpretador atual,
   o script tenta reiniciar usando `.venv\Scripts\python.exe`.
3. **Verificação de dependências** — Detecta pacotes ausentes e mostra um
   diálogo de erro com comandos de instalação.
4. **Inicialização do Qt** — Cria `QApplication`, aplica escala de fonte DPI,
   carrega a folha de estilo escura (`dark.qss`) e define o ícone do aplicativo.
5. **Logging** — `configure_logging()` anexa um handler rotativo em
   `%LOCALAPPDATA%\WSLManagerPro\logs\app.log`.
6. **Config e i18n** — Carrega `ConfigManager` (auto-salva em migração de
   esquema) e inicializa o gerenciador de idiomas com a preferência persistida.
7. **Elevação de admin** — Se não estiver executando como administrador e a
   configuração solicitar, o usuário é perguntado se deseja reiniciar elevado.
   Escolher "Não" desativa `run_as_admin` e continua em modo limitado.
8. **MainWindow** — Cria e mostra a janela principal.
9. **Loop de eventos** — Entra em `app.exec()` até a janela ser fechada.
...
- A **UI nunca bloqueia** em operações longas — todo o trabalho pesado
  é delegado para **workers QThread** que se comunicam via sinais Qt.
- **`WslEngine`** é o único módulo que dispara processos do sistema. Decodifica
  saída (UTF-16 LE para metacomandos, UTF-8 para bash).
- **`DownloadManager`** oferece retomada HTTP, verificação de checksum e
  extração multi-formato (APPX, bootstrap de Arch).
- **`Catalog Loader`** valida e mescla catálogos locais + remotos.
- **`ConfigManager`** persiste ajustes, estados de download e registro de
  distros instaladas.
...
Funções-chave: `_is_admin()`, `_elevate_windows()`,
`_relaunch_with_workspace_venv()`, `_load_dark_stylesheet()`,
`_detect_missing_runtime_dependencies()`, `_resource_path()`, `main()`.

A função `main()` executa uma inicialização de 13 passos: app ID → import PySide6 →
QApplication → verificação de dependências → escala de fonte → folha de estilo
escura → ícone → logging → config → i18n → aviso de admin → MainWindow →
loop de eventos.

### Pacote Core (`core/`)

**`core/constants.py`** — Constantes centralizadas de timeout, retentativas,
...
Exceções: `WslNotFoundError`, `WslCommandError`. Operações de ciclo de
vida de distros (import/export/unregister/set-default/terminate/shutdown),
execução de comandos em tempo real (`run_command`/`run_command_as_root`),
pipeline de provisionamento pós-instalação (`build_post_install_steps`/
`inject_post_install`) compatível com apt/dnf/zypper/pacman/apk, e geração
de `.wslconfig`. As senhas são gravadas por arquivo temporário dentro
do guest e removidas imediatamente após `chpasswd` — nunca visíveis por `ps`.

**`core/wsl_list_parser.py`** — Funções puras (sem dependência de
subprocess): `parse_wsl_list_verbose()` e `parse_wsl_list_online()`. Tratam
BOM UTF-16 LE, cabeçalhos localizados (en/es/pt) e o marcador `*` de
distro padrão.

**`core/downloader.py`** — `DownloadManager`: download HTTP com streaming,
retomada por Range, até 3 retentativas, callback de progresso, verificação de
checksum (SHA-256/SHA-512/MD5) e cancelamento cooperativo com
`threading.Event`. Também: `extract_appx()` para arquivos APPX/ZIP e
`extract_arch_bootstrap()` para `.tar.zst` (descompressão zstandard → reempacotamento como `tar.gz`).

**`core/catalog_loader.py`** — Dataclass `CatalogLoadResult`. `load_catalog()`
valida e mescla catálogos de distros locais + remotos. Entradas
inválidas são ignoradas com avisos.

### Pacote Utils (`utils/`)

**`utils/config_manager.py`** — `ConfigManager`: JSON persistente em
`%APPDATA%\WSLManagerPro\config.json`. Dataclass `AppConfig` com todos os
ajustes. Migração de esquema v1→v2 com auto-salvamento. Modelos
`InstalledDistro` e `DownloadState`. Validação estrita ao salvar, carga
tolerante com valores padrão.

**`utils/app_logging.py`** — Handler de arquivo rotativo: 2 MB máximo, 5
backups, codificação UTF-8. Sanitização de senhas por convenção.
...
com metadados de `distros.json`), menu de contexto na tabela Dashboard, fluxo
completo do assistente de instalação com suporte a PowerShell externo para
distros legacy. `closeEvent` encerra processos rastreados.

**`ui/dialogs.py`** — Diálogos modais: `UserCreationDialog` (regex de nome de
usuário `^[a-z_][a-z0-9_-]{0,30}$`, senha ≥ 4 caracteres,
checkbox sudo), `DirectoryDialog`, `SwapConfigDialog` e `InstallWizard`
(fluxo guiado de 5 páginas: Seleção de Distro → Caminhos → Conta de Usuário →
Resumo → Progresso). Suporta salvar/carregar perfis e detecção de distros
legacy interativos.

**`ui/icons.py`** — Fábrica programática de `QIcon` com `QPainter`:
running (verde), stopped (cinza), installing (laranja), default (azul).

**`ui/theme.py`** — 9 constantes de cor nomeadas: `COLOR_TEXT`,
...
- `DashboardTab` — Tabela de distros com 7 colunas e controles de cabeçalho.
...
Catálogo JSON estático na raiz do projeto. Cada chave é um ID interno
de distro (ex.: `ubuntu-2404`):
...
| `checksum_file_pattern` | `string?` | Padrão de nome de arquivo no índice de checksum |
...
| `packages` | `string[]?` | Pacotes instalados durante pós-instalação |
| `extract_type` | `string` | Formato: `tar.gz`, `tar`, `tar.xz`, `tar.zst`, `appx`, `zip` |
| `systemd` | `boolean?` | Se o suporte de boot do systemd está disponível |
...
| `update_repo_url` | `https://github.com/wilkinbarban/WSL-Manager-Pro/releases` | URL do GitHub Releases usada para verificar atualizações |
| `memory_limit_gb` | `4` | Limite de memória da VM WSL 2 (1–256 GB) |
| `swap_size_gb` | `2` | Espaço swap WSL 2 (0–128 GB) |
| `processors` | `2` | Núcleos lógicos de CPU para WSL 2 (1–256) |
| `localhost_forwarding` | `true` | Encaminhamento de portas do Windows para WSL |
...
| `download_states` | `{}` | Metadados de retomada de download |
| `installed_distros` | `[]` | Registro de distros instaladas por este app |
...
- **Seletor de idioma** — Troca o idioma da UI em tempo real (en / es / pt).

### Aba Dashboard

- Tabela de 7 colunas: ícone de status, nome, estado, versão WSL, marcador
  de padrão, status de usuário, botão de ação.
...
- Estado vazio com Retry Detection quando o WSL não está disponível.

### Aba Manage

- **Import:** caminho tar, nome WSL, diretório de instalação → `wsl --import`.
- **Export:** seletor de distro + caminho de saída → `wsl --export`.
...
- Limites de recursos WSL 2: memória, swap, processadores, encaminhamento localhost,
  tempo ocioso de VM (toggle avançado).
- Botão **Apply & Write .wslconfig** (worker em segundo plano).
- Botão **Export Diagnostic Bundle** (ZIP com versão do app, cauda de log,
...
- **Página 5** — Registro de progresso em tempo real com transições de etapa
  (Download → Extract → Import → Post-install → Complete).

Para Oracle Linux e SUSE Enterprise (distros legacy interativos), o
assistente pode delegar para uma janela externa de PowerShell.

---

## Motor WSL (`WslEngine`)

- Resolve o caminho de `wsl.exe` a partir de `System32`, `SysNative` ou `PATH`.
...
- Suporta todos os subcomandos de `wsl.exe`: import, export, unregister,
...
- **Tamanho de chunk:** 128 KiB.
- **Retomada:** HTTP Range; detecta automaticamente tamanho de arquivo existente.
...
- **Extração de arquivos:** APPX (ZIP → localizar rootfs tar) e bootstrap
  do Arch (`.tar.zst` → reempacotar como `tar.gz` simples).

---

## Threads de Trabalho (Qt)

Todos os workers herdam de `BaseWorker` (ou `CancellableWorker`) e rodam
...
──────────────────────────────────────────────────────
log_message(str)        → adicionar ao console de log
error_occurred(str)     → mostrar erro no log + barra de status
...
O aplicativo suporta **três idiomas** com troca em tempo real (sem reinício):
...
- **Compatível com PyInstaller** por meio de `sys._MEIPASS`.
- **Degradação elegante:** erros de parsing JSON produzem catálogos vazios
  (as chaves usam fallback em inglês).
...
- **Console de log da UI:** suporta filtro por palavra-chave, copiar tudo,
  copiar seleção e codificação de cores HTML.
- **Segurança:** senhas nunca são registradas em operação normal
  (aplicado por convenção em cada ponto de chamada).

### Pacote de Diagnóstico (ZIP)

Gerado na aba Settings. Conteúdo:
- `README.txt` — Timestamp, versão do app, status de comandos, aviso
  de privacidade.
...
Sem segredos por design. O README inclui um lembrete para revisar o
conteúdo do log antes de compartilhar.

---

## Privilégios e Segurança

- **Elevação de administrador:** o aplicativo pergunta se deseja reiniciar
  elevado ao iniciar. Se o usuário recusar, ele roda em modo limitado
  (somente leitura) e desativa a preferência `run_as_admin`.
- **Senhas Linux:** durante pós-instalação, a senha é gravada em
  arquivo temporário dentro do guest e removida imediatamente depois
  que o `chpasswd` a lê. Nunca fica visível por `ps` nem é registrada no
  console.
- **Validação de nome de usuário:** aplicada tanto na UI
  (`UserCreationDialog`) quanto no motor (`build_post_install_steps`) com
  a expressão regular `^[a-z_][a-z0-9_-]{0,30}$`.
- **Sem segredos hardcoded:** o código não contém chaves de API, tokens nem
  credenciais fixas.
...
`WSLManagerPro.exe` independente em `dist/`. O executável inclui todos
os módulos Python (arquivo `.pyz`), `distros.json`, traduções
...
Os **42 testes passam** sem exigir `wsl.exe` — os parsers são funções
puras e os testes de downloader/engine usam mocks.
...
Consulte [`ROADMAP.md`](ROADMAP.md) para o plano de desenvolvimento multifase
completo (150+ tarefas em 8 fases: refatoração de UI, ferramentas,
observabilidade, configuração, privilégios, UX, empacotamento e melhorias do
motor).
